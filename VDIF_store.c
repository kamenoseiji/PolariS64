	...
	char	**argv )		// Pointer to Arguments
{
    ...
    int     thredID;                    // thred ID (= stream index)
...
//------------------------------------------ Open Socket to OCTAVIA
    ...
    thredID    = ((buf[12] & 0x03) << 8 ) + buf[13];
    printf("frameID = %06d : thredID = %d\n", frameID, thredID);
    while(thredID < NST){
		rv = recv(sock, buf, sizeof(buf), 0);
        thredID    = ((buf[12] & 0x03) << 8 ) + buf[13];
    }
