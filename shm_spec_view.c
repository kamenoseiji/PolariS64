	...
	char	**argv )		// Pointer to Arguments
{
    ...
    int     bunchNum;                   // Number of channels to bind
	float	*xspec_ptr;					// Pointer to Cross Spectra
	float	*spec_ptr;					// Pointer to channel-averaged spectrum
...
//------------------------------------------ K5 Header and Data
	...
	for(ch_index=0; ch_index<MAX_CH_VIEW; ch_index ++){
		freq_ptr[ch_index] = (float)(freq_incr* (ch_index - 0.5));
	}
...
//--------- Vecter bunching
int bunchVec(
    int     vecLen,         // Length of the vector
    int     bunchNum,       // Number to bunch
    ...
    float  *out_ptr)       // Pointer to the output vector
{
    ...
    memset(out_ptr, 0x00, outNum* sizeof(float)); index = 0;
